...
#SBATCH --job-name=bigshot_linear_amae_base_frozen
#SBATCH --output=/mnt/work/bird2vec/logs/bigshot/full_bigshot_linear_amae_base_frozen_%N_%a.log
...
CONFIG_PATH="experiment=paper/bigshot/audioMAE/frozen/linear/${DATASETS[$DATASET_IDX]}_base.yaml"
echo "Running experiment: ${CONFIG_PATH} with seed: ${SEED}"
DATASET_NAME=${DATASETS[$DATASET_IDX]}
scontrol update job=$SLURM_JOB_ID name="bigshot_base_frozen_linear_amae_full_${DATASET_NAME}_seed${SEED}_${SLURM_ARRAY_TASK_ID}"

srun python finetune.py \
    ${CONFIG_PATH} \
    seed=${SEED} 
    #module.network.pretrained_weights_path="'/mnt/work/bird2vec/logs_pretrain_audioset_MAE/pretrain_xcl_wave/runs/XCL/AudioMAE/2024-11-24_212406/callback_checkpoints/AudioMAE_XCL_epoch=99.ckpt'"
