...
class MultiHeadedAttention(nn.Module):
    """
    Adapted from the annotated transformers. Took out query,key,val parameters in
    forward method because we don't need cross-attention. Used alibi positional
    embeddings.
    """
    ...
    def forward(self, x, padding_mask=None):
        """
        Multi-Head attention adds an extra dimension, h, to our residual stream,
        which had dimensions [n_batches, seq_len, d_model].
        """
        if padding_mask is not None:
        ...
        query, key, value = [
            ...
            for lin in self.linears[:3]
        ]
        # projections from d_model to d_model, then splits and transposes
        # query.size() == [n_batches, h, seq_len, d_k]

        x, self.attn = attention(
            query, key, value, self.alibi_bias, padding_mask=padding_mask, dropout=self.dropout
        )
...
class TransformerStack(nn.Module):
    ...
    def __init__(self, d_model, number_heads, bias, d_ff, dropout):
        ...
        self.attn = MultiHeadedAttention(
            d_model=d_model, number_heads=number_heads, alibi_bias=bias, dropout=dropout
        )
